...
void CEventDispatch::_CheckTimer() {
    ...
    uint64_t curr_tick = GetTickCount(); // 获取当前时间戳
    ...
    for (it = timer_list_.begin(); it != timer_list_.end();) {
        ...
        TimerItem *pItem = *it; // 获取当前迭代器指向的定时器项
        it++; // 在回调中迭代器可能会被删除，所以我们应该在回调前递增迭代器
              // iterator maybe deleted in the callback, so we should increment
              // it before callback
